...
static func _adapt_v1_xy_to_internal(human: Dictionary) -> Dictionary:
	...
	return {
		...
		"data": {
			...
			"family": str(human.get("family", "")).strip_edges().to_lower(),
			"display_family": str(human.get("display_family", human.get("family", ""))).strip_edges(),
			"mode": str(human.get("mode", "")).strip_edges().to_lower(),
			"player_buckets": human.get("player_buckets", human.get("player_modes", [])),
			"playstyle_tags": human.get("playstyle_tags", human.get("game_types", [])),
...
static func build_internal_map(human: Dictionary) -> Dictionary:
	...
	var internal := {
		...
		"family": str(source.get("family", "")).strip_edges().to_lower(),
		"display_family": str(source.get("display_family", source.get("family", ""))).strip_edges(),
		"mode": str(source.get("mode", "")).strip_edges().to_lower(),
		"player_buckets": source.get("player_buckets", source.get("player_modes", [])),
		"playstyle_tags": source.get("playstyle_tags", source.get("game_types", [])),
