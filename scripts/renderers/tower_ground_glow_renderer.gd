...
class_name TowerGroundGlowRenderer
extends Node2D
...
func _ready() -> void:
	...
	SFLog.allow_tag("TOWER_GLOW_MODEL")
...
func set_model(model: Dictionary) -> void:
	...
	_towers = towers_v as Array if typeof(towers_v) == TYPE_ARRAY else []
	...
	if not _towers.is_empty() and typeof(_towers[0]) == TYPE_DICTIONARY:
		var t0: Dictionary = _towers[0] as Dictionary
		var control_v: Variant = t0.get("control_hive_ids", t0.get("required_hive_ids", []))
		if typeof(control_v) == TYPE_ARRAY:
			sample_control_ids = control_v as Array
	SFLog.log_on_change_payload("TOWER_GLOW_MODEL", "%d|%d|%d" % [_towers.size(), _hives_by_id.size(), sample_control_ids.size()], {
		"towers_count": _towers.size(),
