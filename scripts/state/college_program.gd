class_name CollegeProgram
extends RefCounted

const ScholasticTypesScript = preload("res://scripts/state/scholastic_types.gd")

static func new_college_program(program_id: String, identity: Dictionary = {}) -> Dictionary:
	var clean_id: String = ScholasticTypesScript.normalize_id(program_id)
	if clean_id.is_empty():
		clean_id = ScholasticTypesScript.normalize_id(str(identity.get("university_name", "university")))
	return {
		"college_program_id": clean_id,
		"university_name": ScholasticTypesScript.clean_text(str(identity.get("university_name", "")), 96),
