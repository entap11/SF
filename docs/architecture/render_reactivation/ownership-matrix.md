...
| --- | --- | --- | --- | --- | --- | --- |
| Player identity and device sessions | Rank/identity service | Rank PostgreSQL | Rank/identity authenticated endpoints | Rank/identity issues session receipts and player JWTs | Authenticated route capability; minimum client; credential availability | Implemented/certified default-off; iOS 4.7.1 plugin builds and links, physical protected-key acceptance remains open |
| Device private credential | Player device secure hardware/store | iOS Keychain/Secure Enclave or Android Keystore | Native credential bridge only; private key never enters GDScript/backend | Device signature proves challenge possession; Rank validates | Release-export adapter gate | Android integrated; iOS 4.7.1 device/simulator link PASS, physical Secure Enclave matrix pending |
...
| Synchronous match result | Trusted match-authority worker | VS result/verification tables | Worker replays exact stream or validates lifecycle | Worker signs immutable result; VS retains result/receipt | Match verification plus mode gate | Certification worker is aligned to the exact 4.7.1 candidate and passed pre/post-rollback managed replay; Production worker remains absent |
