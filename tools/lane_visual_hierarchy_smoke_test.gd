...
func _initialize() -> void:
	...
	_expect_true(int(embedded_profile.get("z_index", 0)) <= -12, "Embedded lanes should sink well into the floor layer")
	_expect_true(int(active_profile.get("z_index", 0)) > 0, "Active lanes should lift above floor-level lanes")
