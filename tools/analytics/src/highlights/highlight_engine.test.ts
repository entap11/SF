...
assert.ok(moneyPayload.acquisition_link.universal_url.startsWith("https://swarmfront.com/play?"));
assert.ok(moneyPayload.acquisition_link.universal_url.includes("utm_campaign=social_highlight"));
assert.equal(moneyPayload.acquisition_link.app_store_url_ios, "https://apps.apple.com/app/idXXXXXXXXX");
assert.equal(moneyPayload.highlight_video.status, "queued");
assert.equal(moneyPayload.highlight_video.source, "deterministic_replay_render");
assert.ok(moneyPayload.highlight_video.video_url.endsWith("/match_money_50_comeback/ABC123.mp4"));
assert.equal(moneyPayload.highlight_video.cta.link_url, moneyPayload.acquisition_link.universal_url);
assert.equal(asyncPayload.highlight_video.status, "skipped");

const discordJob = tier1Jobs.find((job) => job.destination === "discord:official-highlights");
assert.equal(discordJob?.payload.video_url, moneyPayload.highlight_video.video_url);
assert.equal(discordJob?.payload.acquisition_url, moneyPayload.acquisition_link.universal_url);
