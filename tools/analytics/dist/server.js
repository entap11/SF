...
function enrichCountry(raw, req) {
...
function getEventsFromBody(body) {
...
async function handleBatch(req, res) {
    ...
    if (validEvents.length > 0) {
        ...
        try {
            ...
            const summary = await ingestValidatedEvents(client, validEvents.map((entry) => entry.event));
...
app.post("/v1/events/batch", asyncHandler(async (req, res) => {
    await handleBatch(req, res);
}));
app.post("/v1/events/single", asyncHandler(async (req, res) => {
...
app.post("/admin/rollups/recompute", requireAdminAuth, asyncHandler(async (req, res) => {
    const date = typeof req.body?.date === "string" ? req.body.date : typeof req.query.date === "string" ? req.query.date : null;
    const start = typeof req.body?.start === "string" ? req.body.start : typeof req.query.start === "string" ? req.query.start : null;
