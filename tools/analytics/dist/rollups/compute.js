...
async function clearDateRows(client, table, dateColumn, date) {
    await client.query(`DELETE FROM ${table} WHERE ${dateColumn} = $1::date`, [date]);
}
async function computeDailyRollup(client, date) {
...
async function computeDailyBuffRollup(client, date) {
...
async function computeDailyMatchWinners(client, date) {
...
async function computeDailyStabilityRollup(client, date) {
...
async function computeRetention(client, date) {
    const windows = [1, 7];
...
export async function computeRollupsForDate(pool, date) {
...
export async function computeRollupsForRange(pool, startDate, endDate) {
    let date = startDate;
    while (date <= endDate) {
