...
function weightedScore(scoreRaw, weight) {
    return Math.round(clamp01(scoreRaw) * weight);
}
function bandedGoldilocks(value, range) {
    if (value < range.min) {
...
function factorFromBand(band) {
    return { label: band.label, score_raw: band.scoreRaw };
}
export function computeSwarmDensity(swarmsSentTotal, durationSeconds) {
    if (durationSeconds <= 0) {
        return 0;
    }
    return Math.max(0, swarmsSentTotal) / durationSeconds;
}
export function computeDurationBand(mode, durationSeconds, config = defaultHighlightConfig) {
    return bandedGoldilocks(durationSeconds, config.goldilocksRanges.duration[mode]);
}
export function computeSwarmDensityBand(mode, playerCount, swarmsSentTotal, durationSeconds, config = defaultHighlightConfig) {
...
export function computeWatchabilityPass(matchAnalytics, config = defaultHighlightConfig) {
    ...
    if (analytics.closeness_factor.label === "severe_blowout" &&
        analytics.closeness_factor.score_raw < config.scoreThresholds.severeBlowoutClosenessRaw) {
        return false;
    }
    return true;
}
function stakeScoreRaw(matchAnalytics, config) {
...
function buffScoreRaw(buffCount, config) {
...
function autoPostTier(excitementScore, watchabilityPass, config) {
    if (!watchabilityPass) {
        return "no_auto_post";
    }
...
export function scoreMatch(matchAnalytics, config = defaultHighlightConfig) {
    const { match, analytics } = matchAnalytics;
    const swarmDensityBand = computeSwarmDensityBand(match.mode, match.player_count, match.swarms_sent_total, match.duration_seconds, config);
