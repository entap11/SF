...
function priorityFor(key, config) {
    return config.badgePriorities[key] ?? 1;
}
function matchBadge(key, name, tier, config) {
    return {
        badge_key: key,
        badge_name: name,
...
function milestoneBadge(base, playerId, config) {
    return {
        ...base,
...
export function deriveCandidateBadges(match, players, analytics, contentFlags, config = defaultHighlightConfig) {
    const badges = [];
    ...
    if (match.player_count >= config.badgeCriteria.fullChaosMinPlayers &&
...
export function selectBadges(candidateBadges, config = defaultHighlightConfig) {
...
export function selectHeadlineBadge(selectedBadges) {
    return selectedBadges.length > 0 ? selectedBadges[0] : null;
}
export function buildDerivedBadges(candidateBadges, config = defaultHighlightConfig) {
    return {
